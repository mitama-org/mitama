...
class TestConfig(unittest.TestCase):
    def test_initialize(self):
        conf = Config(
            Path(os.path.dirname(__file__)) / 'test_apps',
            {
                'a': 'alice',
                'b': 'bob'
            }
        )
        self.assertEqual(conf._project_dir, Path(os.path.dirname(__file__)) / 'test_apps')
        self.assertEqual(conf._sqlite_db_path, Path(os.path.dirname(__file__)) / 'test_apps/db.sqlite3')
